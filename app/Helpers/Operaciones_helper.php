...
function getTipoUsuario()
{
    ...
    $query = "SELECT id,nombre FROM  tipo_usuarios WHERE delated = 0";
...
function getTipoEvaluacion(){
    ...
    $query = "SELECT id,nombre from tipo_evaluacion WHERE delated = 0";
    $resultado = $usermodel ->query($query);
    // Lo comvertimos del tipo array 
    $rowArray = $resultado -> getResult();
    return ($rowArray);
...
function getNivel(){
    ...
    $query = "SELECT id,nombre from niveles_evaluacion WHERE delated = 0";
    $resultado = $usermodel ->query($query);
    // Lo comvertimos del tipo array 
    $rowArray = $resultado -> getResult();
    return ($rowArray);
...
function getleccion(){
    ...
    $query = "SELECT id,nombre from lecciones_evaluacion WHERE delated = 0";
    $resultado = $usermodel ->query($query);
    // Lo comvertimos del tipo array 
    $rowArray = $resultado -> getResult();
    return ($rowArray);
...
{//Funcion para obtener un numero de total de evaluaciones por nivel 
    ...
    $query = "SELECT * from evaluaciones where tipo_evaluacion = $id_tipo_evaluacion AND nivel = $nivel AND delated = 0";
    $resultado = $usermodel ->query($query);
    // Lo comvertimos del tipo array 
    $rowArray = $resultado -> getResult();
...
{
    $usermodel = new Niveles_evaluacion($db);
    $query = "SELECT nombre from niveles_evaluacion WHERE id = $id_nivel AND delated = 0";
    $resultado = $usermodel ->query($query);
...
function getTipoEvaluacionEspecifico($id_tipo_evaluacion)
{
    ...
    $usermodel = new Tipo_evaluacion($db);
    $query = "SELECT id,nombre from tipo_evaluacion WHERE id = $id_tipo_evaluacion AND delated = 0";
    $resultado = $usermodel ->query($query);
    // el getrow nos regresa una sola fila por eso no utlizamos el getResult que nos regresa un array luego no especificamos el lugar
    $row = $resultado->getRow();
    return ($row);
...
{//Funcion para obtener un numero de total de evaluaciones por seccion 
    $usermodel = new Evaluaciones_model($db);
    $query = "SELECT * from evaluaciones where tipo_evaluacion = $id_tipo_evaluacion AND nivel = $id_nivel AND leccion = $id_leccion AND delated = 0";
    $resultado = $usermodel ->query($query);
...
function getValorTotalPreguntas($id_evaluacion)
{
    ...
    $usermodel->SELECT('(SELECT SUM(valor) FROM preguntas WHERE idEvaluacion= '.$id_evaluacion.' AND delated = 0) as v', FALSE);
...
//Funcion para obtener el nombre de usuario que creo una evaluacion     
{
    ...
    $query = "SELECT nombre,apellido_paterno,apellido_materno from usuarios where id = $id_usuario AND delated = 0";
     $resultado = $usermodel ->query($query);
...
function getTipoPreguntas()     
{
    ...
    $query = "SELECT id,nombre from tipo_preguntas WHERE delated = 0";
    $resultado = $usermodel->query($query);
    $rowArray = $resultado->getResult();
    return($rowArray);
 ...
 function getPreguntaOpcion_multiple($id_evaluacion,$id_pregunta)
{
    ...
    $query = "SELECT idEvaluacion,idPregunta,valor1,valor2,valor3,valor4 from pregunta_opcion_multiple where idEvaluacion=$id_evaluacion and idPregunta=$id_pregunta AND delated = 0";
    $resultado = $usermodel->query($query);
    $rowArray = $resultado -> getRow();
    return($rowArray);
}
 function getPreguntaOpcion_audio($id_evaluacion,$id_pregunta)
{
    ...
    $query = "SELECT idEvaluacion,idPregunta,nombre_audio,ruta_audio from pregunta_opcion_audio WHERE idEvaluacion=$id_evaluacion AND idPregunta=$id_pregunta AND delated = 0";
    $resultado = $usermodel->query($query);
    $rowArray = $resultado -> getRow();
    return($rowArray);
...
function getPreguntaOpcion_video($id_evaluacion,$id_pregunta)
{
    ...
    $query = "SELECT idEvaluacion,idPregunta,nombre_video,ruta_video FROM pregunta_opcion_video WHERE idEvaluacion=$id_evaluacion AND idPregunta=$id_pregunta AND delated = 0";   
    $resultado = $usermodel->query($query);
    $row = $resultado -> getRow();
    if(empty($row)){
        $ruta = null;
...
function getCategoriaEvaluacion()
{
    ...
    $query = "SELECT id,nombre from categorias_evaluaciones AND delated = 0";
    $resultado = $usermodel->query($query);
    $rowArray = $resultado->getResult();
    return($rowArray);
}
...
{
    $usermodel = new Categorias_Evaluaciones($db);
    $query = "SELECT nombre from categorias_evaluaciones WHERE id = $id_categoria_evaluacion AND delated = 0";
    $resultado = $usermodel->query($query);
    $row = $resultado->getRow();
    $nombre = $row->nombre;
    return($nombre);
